class BallPlayer:
    def __init__(
            self, name: str, number: int, goals: int, passes: int, minutes: int
    ):
        self.name = name
        self.number = number
        self.goals = goals
...
def most_goals(players: list[BallPlayer]) -> str:
    return sorted(players, key=lambda p: p.goals)[-1].name


def most_points(players: list[BallPlayer]) -> tuple:
    player = sorted(players, key=lambda p: p.goals + p.passes)[-1]
    return (player.name, player.number)


def least_minutes(players: list[BallPlayer]) -> BallPlayer:
    return sorted(players, key=lambda p: p.minutes)[0]
