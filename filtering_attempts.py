class CourseAttempt:
    ...
    def __str__(self):
        return f"{self.student_name}, grade for the course " \
            f"{self.course_name} {self.grade}"


def accepted(attempts: list[CourseAttempt]) -> filter:
    return filter(lambda ca: ca.grade >= 1, attempts)


def attempts_with_grade(attempts: list[CourseAttempt], grade: int) -> filter:
    return filter(lambda ca: ca.grade == grade, attempts)


def passed_students(attempts: list[CourseAttempt], course: str):
    filtered = filter(
        lambda ca: ca.course_name == course and ca.grade > 0, attempts)
    return sorted(map(lambda ca: ca.student_name, filtered))
